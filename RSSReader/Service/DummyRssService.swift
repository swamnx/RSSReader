...
class DummyRssService: RssServiceProtocol {
    ...
    func searchFeed(url: URL, completionHandler: (_ error: String?, _ data: SearchFeed?) -> Void) {
        let searchFeed = SearchFeed.init(icon: nil, title: "Test Feed record", url:  "https://www.nasa.gov/rss/dyn/breaking_news.rss", categories: ["dd646", "dd", "dfsfs"])
    ...
    func searchFeedWithoutCategories(url: URL, completionHandler: (_ error: String?, _ data: SearchFeed?) -> Void) {
        let searchFeed = SearchFeed.init(icon: nil, title: "Test Feed record", url: "https://www.nasa.gov/rss/dyn/breaking_news.rss", categories: [])
        completionHandler(nil, searchFeed)
    }
